...
M.config = {
    ...
    callback = function(text)
        vim.notify(text, vim.log.levels.WARN, { title = "Hardtime", timeout = M.config.timeout })
    end,
}

function M.migrate_old_config(config)
    -- Convert array elements in disabled_filetypes to key-value pairs
    if config.disabled_filetypes and type(config.disabled_filetypes) == "table" then
        local new_filetypes = {}
        for k, v in pairs(config.disabled_filetypes) do
            if type(k) == "number" and math.floor(k) == k and k >= 1 and type(v) == "string" then
    ...
    local options_to_process = { "resetting_keys", "restricted_keys", "disabled_keys", "hints" }
